    ...
    case _types__WEBPACK_IMPORTED_MODULE_1__["UmapMessageType"].Execute:
      {
        try {
          ...
          executor.execute(payload.data, epochNumber => {
            sendProgress(epochNumber, executor.embedding);
          }).then(embedding => sendComplete(embedding)).catch(err => sendError(err));
        } catch (err) {
...
const sendProgress = (epochNumber, embedding) => {
  ;
  self.postMessage({
...
class UmapExecutor {
  ...
  async execute(data, onProgress) {
    return this._umap.fitAsync(data, epochNumber => {
...
/******/ });
//# sourceMappingURL=main.b51dd12a8e823a0764c8.worker.js.map