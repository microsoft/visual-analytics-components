 ...
 */
export function createTree(items: SidebarItem[]): TreeNode[] {
...
function childHash(children: TreeNode[]): Record<string, TreeNode> {
	const result: Record<string, TreeNode> = {}
	children.forEach(child => (result[child.pathKey] = child))
	return result
}

function childrenOf(tree: TreeNode): TreeNode[] {
	return Object.keys(tree.children).map(k => tree.children[k])
}

function isLeaf(tree: TreeNode): boolean {
